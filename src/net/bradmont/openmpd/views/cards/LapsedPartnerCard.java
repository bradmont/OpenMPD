...
public class LapsedPartnerCard extends NotificationCard {

	public LapsedPartnerCard(BaseActivity a, Notification n, Contact contact, ContactStatus status){
	...
	public View buildCardContent(Context context) {
        ...
        ((ImageView) view.findViewById(R.id.stripe)).setBackgroundResource(R.color.red);
        ...
        String title_string = String.format(
                context.getResources().getString(R.string.lapsed_partner_title),
        ...
        if (partnership == R.string.monthly ){
            text = String.format("%s %s has lapsted ($%.2f/mo, more than 3 months late)",
                    contact.getString("fname"),
                    contact.getString("lname"),
                    status.getFloat("giving_amount")/100f);
        } else if (partnership == R.string.regular){
            text = String.format("%s %s has lapsted ($%.2f/%dmo, more than 3 months late)",
                    contact.getString("fname"),
                    contact.getString("lname"),
                    status.getFloat("giving_amount")/100f,
